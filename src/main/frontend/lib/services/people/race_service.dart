...
Future<List<Race>> fetchRaces() async {
  final response = await http.get(Uri.parse('${Api.baseUrl}/races'));
...
Future<Race> fetchRace(int id) async {
  final response = await http.get(Uri.parse('${Api.baseUrl}/races/$id'));
...
Future<bool> editRace(
  ...
  final response = await http.put(
    Uri.parse('${Api.baseUrl}/races/$id'),
    headers: {'Content-Type': 'application/json'},
    body: jsonEncode({
      "id": id,
      "name": name,
...
Future<void> deleteRace(int id) async {
  final response = await http.delete(Uri.parse('${Api.baseUrl}/races/$id'));
