...
Future<List<City>> fetchCities(String uuid) async {
  final response = await http.get(
    Uri.parse('${Api.baseUrl}/cities/campaign/$uuid'),
  );
...
Future<City> fetchCity(int id) async {
  final response = await http.get(Uri.parse('${Api.baseUrl}/cities/$id'));
...
Future<bool> editCity(
  ...
  final response = await http.put(
    Uri.parse('${Api.baseUrl}/cities/$id'),
    headers: {'Content-Type': 'application/json'},
    body: jsonEncode({
      "id": id,
      "name": name,
...
Future<void> deleteCity(int id) async {
  final response = await http.delete(Uri.parse('${Api.baseUrl}/cities/$id'));
