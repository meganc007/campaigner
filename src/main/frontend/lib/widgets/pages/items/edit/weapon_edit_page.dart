...
class _WeaponEditPageState extends State<WeaponEditPage> {
  ...
  void initState() {
    ...
    _goldValueController.text = widget.weapon.goldValue!.toString();
    _silverValueController.text = widget.weapon.silverValue!.toString();
    _copperValueController.text = widget.weapon.copperValue!.toString();
    _weightController.text = widget.weapon.weight!.toString();
    _numberOfDiceController.text = widget.weapon.numberOfDice!.toString();
    _damageModifierController.text = widget.weapon.damageModifier!.toString();
    _isMagical = widget.weapon.isMagical!;
    _isCursed = widget.weapon.isCursed!;
    _notesController.text = widget.weapon.notes ?? '';
  }
  ...
  Future<void> _loadInitialData() async {
    try {
      ...
      setState(() {
        ...
        _selectedWeaponType = _weaponTypes.firstWhere(
          (wt) => wt.id == widget.weapon.fkWeaponType,
          orElse: () => _weaponTypes.first,
        );
        _selectedDamageType = _damageTypes.firstWhere(
          (dt) => dt.id == widget.weapon.fkDamageType,
          orElse: () => _damageTypes.first,
        );
        _selectedDiceType = _diceTypes.firstWhere(
          (dice) => dice.id == widget.weapon.fkDiceType,
          orElse: () => _diceTypes.first,
        );
  ...
  Future<void> _onWeaponTypeChanged(WeaponType? newWeaponType) async {
    setState(() {
      _selectedWeaponType = newWeaponType;
    });
  }

  Future<void> _onDamageTypeChanged(DamageType? newDamageType) async {
    setState(() {
      _selectedDamageType = newDamageType;
    });
  }

  Future<void> _onDiceTypeChanged(DiceType? newDiceType) async {
    setState(() {
      _selectedDiceType = newDiceType;
    });
  }
  ...
  Future<void> _submitForm() async {
    ...
    final success = await editWeapon(
      widget.uuid,
      widget.weapon.id,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Edit ${widget.weapon.name}".toUpperCase())),
      body: Padding(
              ...
              child: Column(
                children: [
                  ...
                  EntityDropdown<WeaponType>(
                    ...
                    getLabel: (wt) => wt.name,
                    onChanged: _onWeaponTypeChanged,
                  ),
                  ...
                  EntityDropdown(
                    ...
                    getLabel: (dt) => dt.name,
                    onChanged: _onDamageTypeChanged,
                  ),
